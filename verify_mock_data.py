...
def verify_files():
    ...
    print(f"\nSpatial dimensions:")
    print(f"  Longitude: {ds.dims['longitude']} points ({ds['longitude'].values[0]:.1f}° to {ds['longitude'].values[-1]:.1f}°)")
    print(f"  Latitude: {ds.dims['latitude']} points ({ds['latitude'].values[0]:.1f}° to {ds['latitude'].values[-1]:.1f}°)")
    ...
    print(f"  Ensemble members per file: 50")
    print(f"  Spatial resolution: 361 x 720 (lat x lon)")
